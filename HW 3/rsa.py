...
def sign(key, plaintext):
	if verbose: print(f"signing message \"{plaintext}\".")
...
def checkSign(key,plaintext,signature):
	...
	if verbose: print(f"plaintext: {plaintext}\nhash: {hash}\ndecrypt: {decrypt(key, signature)}")

	if hash == decrypt(key, signature):
		return True
	return False
